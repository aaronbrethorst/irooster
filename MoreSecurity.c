	...
	// See also the companion routine, WriteDictionaryToDescriptor, below.
{
	...
	err = EXXXToOSStatus( UNIXRead(fdIn, &dictSize, sizeof(dictSize), NULL) );
	if (err == noErr) {
		if (dictSize == 0) {
	...
	if (err == noErr) {
		err = EXXXToOSStatus( UNIXRead(fdIn, dictBuffer, (size_t) dictSize, NULL) );
	}
	if (err == noErr) {
	...
	// how much to read.
{
	...
	// dict with one UNIXWrite call, which definitely speeds 
	...
	if (err == noErr) {
		...
		*((size_t *) dictBuffer) = (size_t) dictSize;
		...
		err = EXXXToOSStatus( UNIXWrite(fdOut, dictBuffer, sizeof(size_t) + dictSize, NULL) );
	}
			...
			*response = receivedResponse;
			...
			if (err == 0) {
				...
				for (descIndex = 0; descIndex < descCount; descIndex++) {
					...
					err = UNIXReadDescriptor(fdIn, &thisDesc);
					if (err == 0) {
		...
		if (descArray != NULL) {
			...
			if (err == 0) {
				...
				for (descIndex = 0; descIndex < descCount; descIndex++) {
					...
					if (err == 0) {
						err = UNIXWriteDescriptor(fdOut, thisDesc);
					}
	...
	// command response from the second instance of the tool to fdOut.
{
	...
	if (err == 0) {
		...
		if (err == 0) {
			err = UNIXRead(fdConnToChild, &childPID, sizeof(childPID), NULL);
		}
		...
		if (err == 0) {
			err = UNIXCopyDescriptorToDescriptor(fdIn, fdConnToChild);
		}
		...
		// So, unlike the send side, we can't just call UNIXCopyDescriptorToDescriptor, 
		...
		// send us its PID via fdConnToChild (see the UNIXRead above).
		// Also note that there's no guarantee that the UNIXRead will 
	...
	// See comment in header.
{
	...
	if (err == 0) {
	    if (argc == 1) {
			...
			err = UNIXRead(fdIn, &extAuth, sizeof(extAuth), NULL);
		    if (err == 0) {
			...
			myPID = getpid();
			err = UNIXWrite(fdOut, &myPID, sizeof(myPID), NULL);
	...
	// which tool would be contained must exist
{
	...
	if (err == noErr && !looksOK) {
		...
		if (err == noErr) {
			err = UNIXCopyFile(templateToolPath, toolPath);
		}
	}
	...
	// See comment in header.
{
	...
	if (err == noErr) {	
		err = EXXXToOSStatus( UNIXWrite(fdParent, &extAuth, sizeof(extAuth), NULL) );
	}
	if (err == noErr) {
